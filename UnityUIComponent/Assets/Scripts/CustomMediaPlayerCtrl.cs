...
public class CustomMediaPlayerCtrl : MonoBehaviour {

	public string[] m_playList;
...
//			m_TargetMaterial.GetComponent<MeshFilter>().mesh.uv = vec2UVs;
			m_TargetMaterial.GetComponent<RawImage>().uvRect = new Rect(0, 1, 1, -1);
		}
	...
	{
		if( m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED  || m_CurrentState == MEDIAPLAYER_STATE.END || m_CurrentState == MEDIAPLAYER_STATE.STOPPED)
...
//			}
			if(m_TargetMaterial.GetComponent<RawImage>().texture != m_VideoTexture)
			{
				m_TargetMaterial.GetComponent<RawImage>().texture = m_VideoTexture;
			}
...
//				m_TargetMaterial.GetComponent<MeshRenderer>().material.mainTexture = videoTexture;
			if(m_TargetMaterial)
				m_TargetMaterial.GetComponent<RawImage>().texture = videoTexture;
	...
	public void ChangeStrFile(string fileUrl) {
		this.m_strFileName = fileUrl;
	}
	...
	public void RandomPlayAds() {
		if(this.m_playList.Length != 0) {
			int tempRand = UnityEngine.Random.Range(0, this.m_playList.Length);
			this.m_strFileName = m_playList[tempRand];
		} else {
			this.m_strFileName = m_playList[0];
		}
		this.Load (m_strFileName);
		this.Play();
	}

	public void VideoManagerInit() {
		this.m_strFileName = this.m_playList[0];
		this.Load(m_strFileName);
		this.Play();
	}
